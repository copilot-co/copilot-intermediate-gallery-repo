...
export function GalleryGrid({
  ...
  const endIndex = showAll ? totalPhotos : currentPage * limit;
                ...
                <div className="relative aspect-[4/3] overflow-hidden">
                  ...
                  <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-300">
                    <button
                      ...
                      aria-label={
                        favoritesLoaded
                          ? `${isFavorite ? 'Remove from favorites' : 'Add to favorites'}: ${photo.title}`
                          : `Loading favorite status for ${photo.title}`
                      }
                      aria-pressed={favoritesLoaded ? isFavorite : undefined}
                      className={`p-2 rounded-full backdrop-blur-sm transition-colors disabled:cursor-wait disabled:opacity-70 ${
                        favoritesLoaded && isFavorite
                          ? 'bg-red-500 text-white'
                          : 'bg-white/80 dark:bg-slate-800/80 text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-700'
                      }`}
                    >
                      <Heart className={`h-4 w-4 ${favoritesLoaded && isFavorite ? 'fill-current' : ''}`} />
                    </button>
                    <button
                      type="button"
