...
export function GalleryPageClient({ photos, availableTags }: GalleryPageClientProps) {
  ...
  const handleLoadMore = async () => {
    ...
    // Keep small delay to preserve existing loading UX.
  ...
  return (
    <div className="page-gradient">
      <Hero title="Photo Gallery" description="Browse and discover amazing photography" />
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="relative" ref={filterRef}>
                <button onClick={() => setIsFilterOpen(!isFilterOpen)} className="btn-secondary flex items-center gap-2">
                  <Filter className="h-4 w-4" />
                  Filters
                  {selectedTags.length > 0 && (
                    <span className="bg-blue-500 text-white text-xs rounded-full px-2 py-0.5 ml-1">{selectedTags.length}</span>
                  )}
                  ...
                  <div className="absolute top-full left-0 mt-2 w-80 card-base border border-slate-300 dark:border-slate-600 shadow-lg z-10">
                    <div className="p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        {selectedTags.length > 0 && (
                          <button onClick={clearAllTags} className="nav-link-active text-sm">
                            Clear all
                          </button>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 max-h-60 overflow-y-auto">
                        {availableTags.map((tag) => (
                          <label key={tag} className="flex items-center gap-2 p-2 rounded-lg hover-bg cursor-pointer">
                            <input
                              type="checkbox"
            ...
            <div className="mt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedTags.map((tag) => (
                  <span key={tag} className="status-badge status-active inline-flex items-center gap-1">
                    {tag}
                    <button onClick={() => toggleTag(tag)} className="hover:bg-blue-200 dark:hover:bg-blue-800 rounded-full p-0.5">
                      <X className="h-3 w-3" />
                    </button>
                  </span>
                ))}
